...
export default function SociosPage() {
  ...
  const [carnetSocio, setCarnetSocio] = useState<Socio | null>(null);
  const [clubNombre, setClubNombre] = useState('');
  const [clubLogo, setClubLogo] = useState<string | null>(null);

  async function cargar() {
    setLoading(true);
    const [s, t, c, club] = await Promise.all([
      ...
      supabase.from('clubes').select('nombre, logo_url').single(),
    ]);
    setSocios((s.data || []) as Socio[]);
    setTipos((t.data || []) as TipoCuota[]);
    setCobradores((c.data || []) as Usuario[]);
    if (club.data) { setClubNombre(club.data.nombre || ''); setClubLogo(club.data.logo_url || null); }
    setLoading(false);
  }
  ...
  return (
    <div>
                  ...
                  <tr key={s.id}>
                    ...
                    <td>
                      <div className="actions">
                        ...
                        <button onClick={() => setCarnetSocio(s)} title="Ver carnet QR"><QrCode size={14} /></button>
              ...
              {filtered.map((s) => (
                <div key={s.id} className="socio-card">
                  ...
                  <div className="socio-card-actions">
                    <button onClick={() => router.push(`/estado-cuenta?socio=${s.id}`)}>Estado</button>
                    <button onClick={() => setEditing(s)}>Editar</button>
                    <button onClick={() => setCarnetSocio(s)}><QrCode size={14} /></button>
      ...
      {carnetSocio && (
        <CarnetSocio
          socio={carnetSocio}
          tipoCuota={tiposMap.get(carnetSocio.tipo_cuota_id || '') || 'Sin asignar'}
          logoUrl={clubLogo}
          nombreClub={clubNombre}
          onClose={() => setCarnetSocio(null)}
        />
      )}
    </div>
  );
